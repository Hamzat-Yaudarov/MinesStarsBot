...
async function openMine(ctx) {
  ...
  const kb = new InlineKeyboard().text(cdMin>0 ? `⏳ Через ${Math.floor(cdMin/60)}ч ${cdMin%60}м` : '⛏️ Копать', cdMin>0 ? 'noop' : 'mine:dig');
...
async function doMine(ctx) {
  ...
  const kb = new InlineKeyboard().text('💰 Продать всё', 'mine:sellall');
