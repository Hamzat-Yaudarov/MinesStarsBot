...
export function registerSell(bot) {
  ...
  bot.on('callback_query', async (ctx, next) => {
    ...
    if (data === 'sell_all') {
      ...
      if (isLocked(userId, 'sell')) { await ctx.answerCbQuery('Уже продаём, подождите...', { show_alert: true }); return; }
      await withLock(userId, 'sell', async () => {
        const inv = await getInventory(userId);
        let totalMc = 0;
        const price = (key) => RESOURCES.find(r=>r.key===key).priceMc;
        for (const r of RESOURCES) totalMc += (inv[r.key]||0) * price(r.key);
        if (totalMc <= 0) { await ctx.answerCbQuery('Нечего продавать', { show_alert: true }); return; }
  ...
  bot.on('text', async (ctx, next) => {
    ...
    const { withLock, isLocked } = await import('../utils/locks.js');
    if (isLocked(ctx.from.id, 'sell')) { await ctx.reply('Уже идёт продажа, подождите...'); return; }
    await withLock(ctx.from.id, 'sell', async () => {
